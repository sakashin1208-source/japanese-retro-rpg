...
4. **操作方法**:
   - **移動**: 方向キー (↑ / ↓ / ← / →) または WASD、ゲームパッド D-Pad / 左スティック
   - **決定 / 会話 / 調べる**: `Z` / `Enter` / `Space` / ゲームパッド Aボタン または マウスクリック
   - **取消 / メニュー開閉**: `X` / `Esc` / ゲームパッド Bボタン

---

## 8. Claude Code レビュー対応 ＆ 品質強化改修実績

Claude Code から受領したコードレビューおよび改修設計案（改訂第2版）に基づき、以下の根本改修を実施し、統合テスト全73件合格を達成しました。

| 改修項目 | 内容 | 成果 |
| :--- | :--- | :--- |
| **① 威力計算・乱数正常化** | 敵行動の `power` 乗算適用、係数 1.2 統一、技乱数を SPEC §6.2 準拠の `±3`（通常/敵は `±2`）へ分離、全体技の対象別個別ロール実装 | ボスの大技（妖狐の紅蓮火等）が本来の脅威度を発揮。全体技で対象ごとに異なるダメージブレを再現。 |
| **② フォント同梱・Theme化** | 和風ドットフォント `DotGothic16-Regular.ttf` 同梱、`main_theme.tres` 作成（フォントサイズ28、黒アウトライン4px） | 全1,212文字実測検証済みで他環境での豆腐（□）化を100%防止。SPEC §10.2必須の黒フチ取りを一括適用。 |
| **③ 章移動の双方向化** | `id` / `start` キー整合、解放済み章リスト選択化、未解放理由の動的生成、マップ初期化時の座標復帰 | 実行時エラーを防止し、第2章から第1章への後退を可能に。未解放時は必要な条件をゲーム内で親切に通知。 |
| **④ 移動入力アクション分離** | `move_*`（矢印/WASD/D-Pad/スティック）新設、UI操作（`ui_*`）と完全分離、決定キーを `confirm` に一貫統一 | UIフォーカス移動の汚染を回避。WASD移動およびゲームパッド操作に完全対応。 |
| **⑤ 描画ノード再利用最適化** | コマンド・スキル・ターゲット選択リストで `Label` ノードを再利用する差分更新へリファクタリング | カーソル移動ごとの無駄な `queue_free` / `new` を排除し、描画負荷とメモリ断片化を解消。 |
| **⑥ 統合テスト拡張 (全73件)** | M4（Theme・フォント・画面比率）、受入11（技乱数・敵威力）、受入12（章移動キー・双方向性）、受入13（`ui_accept` 残存ゼロ静的検査）を追加 | **全73件 全件合格（100% PASS）** を確認。 |