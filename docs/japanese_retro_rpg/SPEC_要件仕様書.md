...
1. **ビルド不要**: ビルドツール・パッケージマネージャ・トランスパイルを必須にしない。ソースを配置すればそのまま動くこと。
   - **【2026-08-31 改訂】** 初版は「ローカルサーバを必須にしない」（＝`index.html` の直開き）としていたが、
     再構築では **ES Modules（`<script type="module">`）を採用**する。ES Modules は CORS 制約により `file://` で動作しないため、
     **ローカルサーバ（`python3 -m http.server 8080` 等）を前提とする**。
   - 判断根拠: スマホ実機テストは元々 `start_mobile_server.bat`（`python -m http.server 8080`）でサーバを使用しており、
     実運用上の損失がない。一方 ES Modules はグローバル汚染と読み込み順依存を解消する（`DESIGN_技術設計書.md` §12）。
   - **ビルド工程を持ち込まない**という原則自体は維持する。
...
| 敵スプライト | 128 × 128 | 50種 |
| ボススプライト | 192 × 192 | 9種 |
| 顔ポートレート | 128 × 128 | 29種（主人公3・ボス9・NPC17） |
| **実体合計** | | **166枚**（登録キー197個。うち31個は別名を指すエイリアス） |

> 上記は 2026-08-31 に `tools/export_sprites.html` で全スプライトを実出力して計測した実測値。
> サイズ別内訳: 64px 66枚 / 128px 82枚 / 192px 18枚。アトラス化すると 2044 × 1392 px に収まる。

**全アセットは起動時にオフスクリーンCanvasへ描画してキャッシュする**（毎フレーム生成は禁止）。
生成所要は約20〜40ms（実測）。

**命名の統一（再構築時の必須事項）**: 現行は同一スプライトが2つのキーで登録されている箇所が31件ある
（例: `npc_village_head` と `npc_elder`、`boss_hyoka` と `hyoka`）。日本語名ベースの新命名を追加した際に
`data.js` 側を書き換えずエイリアスで対応した結果と見られる。**再構築時はどちらか一方へ統一すること。**
...
### B-4【2026-08-31 修正済】マップ上のアイテム使用が未配線
...
> **【2026-08-31 追記】修正確認済**。`map.js:328`（タップ）および `map.js:488`（キー操作）の
> 両経路から `useItemOnMap()` が呼ばれることをコード上で確認した。**上記の教訓は再構築時にも引き続き適用すること。**
...
### B-9【2026-08-31 修正済】敵グラフィックの作り込み不足

指摘当時は「敵50種の多く（特に第二章・第三章）が矩形/円を2〜3個重ねただけ」であったが、その後 `graphics.js` が
1,281行 → 2,804行へ拡張され、**`createOutlinedTile()` によるグラデーション・多角形・輪郭線を用いた描画へ全面的に改善済み**。
顔ポートレートも3種（主人公のみ）→ 29種（ボス9体・NPC17名を追加）へ拡充された。

再構築時も**この品質を下回らないこと**を要件とする。実物は `tools/export_sprites.html` の
「一覧シート PNG」で全166枚を一覧確認できる。

### B-10【2026-08-31 撤回・訂正】`image-rendering` の設定は矛盾ではない

> **本項目は誤指摘であった。初版の「`pixelated` に統一せよ」という指示に従ってはならない。**

初版では「JS側 `imageSmoothingEnabled = false` と CSS側 `image-rendering: auto` が矛盾している」と指摘したが、
**両者は異なる段階に作用しており、矛盾ではない。**

| 設定 | 作用する段階 | 本プロジェクトでの影響 |
|---|---|---|
| `ctx.imageSmoothingEnabled = false` | **キャンバス内部**の `drawImage` 時の補間 | スプライトは64px枠に64pxで**等倍描画**されるため、実質的に影響しない |
| CSS `image-rendering` | **キャンバス要素を画面サイズへ拡縮**する段階 | 1280×960 → 実機約1000×750デバイスピクセルへの**縮小**に作用する |

スマホでは常に**縮小**表示となる。ここで `pixelated`（最近傍法）を指定すると、
現在のグラデーション・多角形主体のアート（`createOutlinedTile`）ではピクセルが間引かれ、
**移動時のちらつき・ジャギーが発生して画質が劣化する。**

**確定仕様（変更しないこと）**:

```
ctx.imageSmoothingEnabled = false   // キャンバス内部：スプライトを等倍でクリスプに描画
CSS  image-rendering: auto          // 画面への縮小：スムーズ補間でジャギーを防ぐ
```

なお、本作は厳密なドット絵ではなくグラデーションを用いた HD-2D 風の描画である。
`index.html` の「ピクセルパーフェクト」という訴求文言は実態と合わないため、再構築時は表現を見直すこと。
