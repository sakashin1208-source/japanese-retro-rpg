/**
 * 妖幻奇譚 〜もののけ草子〜 グラフィックエンジン (GraphicsEngine)
 * 全166枚スプライト（タイル25 + 歩行24 + 戦闘12 + NPC17 + 敵50 + ボス9 + ポートレート29）生成＆キャッシュ
 */
import { FONTS, COLORS } from '../core/Constants.js';
...
export class GraphicsEngine {
   ...
   * @param {string} [strokeColor='#0b0810'] 
   * @param {number} [lineWidth=3.5] 
   * @param {CanvasTextAlign} [align='left'] 
   */
  drawCrispText(ctx, text, x, y, fontStr, color = '#ffffff', strokeColor = '#0b0810', lineWidth = 3.5, align = 'left') {
    ctx.save();
    ctx.font = fontStr;
    ctx.textAlign = align;
    ctx.lineJoin = 'round';
  ...
  createOutlinedTile(size, drawFn, outlineColor = '#0b0810', outlineWidth = 2.5) {
    const c = document.createElement('canvas');
    c.width = size;
    c.height = size;
    const ctx = c.getContext('2d');
  ...
  generateNpcSprites() {
    ...
    this.sprites.npc_elder = this.createOutlinedTile(s, (ctx) => {
  ...
  generatePortraits() {
    ...
    this.portraits.ninja = this.createTile(s, (ctx) => {
      ...
      ctx.fillStyle = '#0e121a'; ctx.fillRect(20, 6, 88, 48); ctx.fillRect(14, 20, 100, 54);
      ctx.fillStyle = '#1e2434'; ctx.fillRect(28, 8, 72, 12); ctx.fillRect(18, 24, 14, 44); ctx.fillRect(96, 24, 14, 44);
      ctx.fillStyle = '#4a5870'; ctx.fillRect(20, 24, 88, 8);
      ctx.fillStyle = '#0e121a'; ctx.fillRect(24, 68, 80, 28);
      ctx.fillStyle = '#1c2230'; ctx.fillRect(28, 70, 72, 10);
      ctx.fillStyle = '#f0c8a0'; ctx.fillRect(30, 36, 68, 32);
      ctx.fillStyle = '#111'; ctx.fillRect(34, 44, 24, 4); ctx.fillRect(70, 44, 24, 4);
      ctx.fillStyle = '#200830'; ctx.fillRect(40, 48, 14, 10); ctx.fillRect(74, 48, 14, 10);
      ctx.fillStyle = '#a040d0'; ctx.fillRect(44, 50, 6, 6); ctx.fillRect(78, 50, 6, 6);
      ctx.fillStyle = '#fff'; ctx.fillRect(45, 51, 3, 3); ctx.fillRect(79, 51, 3, 3);
      ctx.fillStyle = '#0a0d14'; ctx.fillRect(12, 96, 104, 32);
      ctx.fillStyle = '#161c28'; ctx.fillRect(16, 98, 96, 12);
      ctx.fillStyle = '#401860'; ctx.fillRect(44, 96, 40, 32);
      ctx.fillStyle = '#682898'; ctx.fillRect(52, 96, 24, 32);
      ctx.fillStyle = '#c0a0e0'; ctx.fillRect(60, 96, 8, 32);
    });

    // --- ボス9体ポートレート ---
    // 1. 赤鬼
    this.portraits.akaoni = this.createTile(sBoss, (ctx) => {
      ctx.fillStyle = '#200808'; ctx.fillRect(0, 0, sBoss, sBoss);
      ctx.strokeStyle = '#881010'; ctx.lineWidth = 6; ctx.strokeRect(3, 3, sBoss - 6, sBoss - 6);
      const sprite = this.sprites.akaoni;
      if (sprite) ctx.drawImage(sprite, 0, 0, sBoss, sBoss);
    });

    // 2. 大天狗
    this.portraits.tengu = this.createTile(sBoss, (ctx) => {
      ctx.fillStyle = '#181008'; ctx.fillRect(0, 0, sBoss, sBoss);
      ctx.strokeStyle = '#884410'; ctx.lineWidth = 6; ctx.strokeRect(3, 3, sBoss - 6, sBoss - 6);
      const sprite = this.sprites.tengu;
      if (sprite) ctx.drawImage(sprite, 0, 0, sBoss, sBoss);
    });

    // 3. 妖狐・茜
    this.portraits.youko = this.createTile(sBoss, (ctx) => {
      ctx.fillStyle = '#200818'; ctx.fillRect(0, 0, sBoss, sBoss);
      ctx.strokeStyle = '#a82060'; ctx.lineWidth = 6; ctx.strokeRect(3, 3, sBoss - 6, sBoss - 6);
      const sprite = this.sprites.youko;
      if (sprite) ctx.drawImage(sprite, 0, 0, sBoss, sBoss);
    });

    // 4. 雪女・氷華
    this.portraits.hyoka = this.createTile(sBoss, (ctx) => {
      ctx.fillStyle = '#081420'; ctx.fillRect(0, 0, sBoss, sBoss);
      ctx.strokeStyle = '#2878a8'; ctx.lineWidth = 6; ctx.strokeRect(3, 3, sBoss - 6, sBoss - 6);
      const sprite = this.sprites.boss_hyoka;
      if (sprite) ctx.drawImage(sprite, 0, 0, sBoss, sBoss);
    });

    // 5. 蛟龍
    this.portraits.mizuchi_boss = this.createTile(sBoss, (ctx) => {
      ctx.fillStyle = '#081820'; ctx.fillRect(0, 0, sBoss, sBoss);
      ctx.strokeStyle = '#208888'; ctx.lineWidth = 6; ctx.strokeRect(3, 3, sBoss - 6, sBoss - 6);
      const sprite = this.sprites.boss_mizuchi;
      if (sprite) ctx.drawImage(sprite, 0, 0, sBoss, sBoss);
    });

    // 6. 酒呑童子
    this.portraits.shuten = this.createTile(sBoss, (ctx) => {
      ctx.fillStyle = '#200808'; ctx.fillRect(0, 0, sBoss, sBoss);
      ctx.strokeStyle = '#a81818'; ctx.lineWidth = 6; ctx.strokeRect(3, 3, sBoss - 6, sBoss - 6);
      const sprite = this.sprites.boss_shuten;
      if (sprite) ctx.drawImage(sprite, 0, 0, sBoss, sBoss);
    });

    // 7. 茨木童子
    this.portraits.ibaraki = this.createTile(sBoss, (ctx) => {
      ctx.fillStyle = '#200c14'; ctx.fillRect(0, 0, sBoss, sBoss);
      ctx.strokeStyle = '#982848'; ctx.lineWidth = 6; ctx.strokeRect(3, 3, sBoss - 6, sBoss - 6);
      const sprite = this.sprites.boss_ibaraki;
      if (sprite) ctx.drawImage(sprite, 0, 0, sBoss, sBoss);
    });

    // 8. 無想影
    this.portraits.musokage = this.createTile(sBoss, (ctx) => {
      ctx.fillStyle = '#080810'; ctx.fillRect(0, 0, sBoss, sBoss);
      ctx.strokeStyle = '#483868'; ctx.lineWidth = 6; ctx.strokeRect(3, 3, sBoss - 6, sBoss - 6);
      const sprite = this.sprites.boss_musokage;
      if (sprite) ctx.drawImage(sprite, 0, 0, sBoss, sBoss);
    });

    // 9. 九尾大妖狐・真茜
    this.portraits.shin_youko = this.createTile(sBoss, (ctx) => {
      ctx.fillStyle = '#280414'; ctx.fillRect(0, 0, sBoss, sBoss);
      ctx.strokeStyle = '#c82868'; ctx.lineWidth = 6; ctx.strokeRect(3, 3, sBoss - 6, sBoss - 6);
      const sprite = this.sprites.boss_shin_youko;
      if (sprite) ctx.drawImage(sprite, 0, 0, sBoss, sBoss);
    });

    // --- 街人NPC全17名ポートレート ---
    const drawScaledNpcPortrait = (spriteKey, borderColor = '#4a3828') => {
      return this.createTile(s, (ctx) => {
        ctx.fillStyle = '#181420'; ctx.fillRect(0, 0, s, s);
        ctx.strokeStyle = borderColor; ctx.lineWidth = 4; ctx.strokeRect(2, 2, s - 4, s - 4);
        const sprite = this.sprites[spriteKey];
        if (sprite) {
          ctx.drawImage(sprite, 0, 0, 64, 64, 16, 16, 96, 96);
        }
      });
    };

    this.portraits.npc_elder = drawScaledNpcPortrait('npc_elder', '#6a5030');
    this.portraits.npc_ohana = drawScaledNpcPortrait('npc_ohana', '#c04868');
    this.portraits.npc_kannushi = drawScaledNpcPortrait('npc_kannushi', '#485888');
    ...
    this.portraits.npc_yugen = drawScaledNpcPortrait('npc_yugen', '#785830');
    ...
    this.portraits.npc_abe = drawScaledNpcPortrait('npc_abe', '#383868');
    this.portraits.npc_hime = drawScaledNpcPortrait('npc_hime', '#a83050');
